# From this lesson, we learn about JS static object: Math. It mainly helps us to
# carry out mathematical calculations. It has a lot of properties and methods.
# Some of the properties and methods we rarely used. So we only learn some
# common methods.
#
# The properties of the Math object are some constants, such as PI, on behalf of
# the approximate value of pi. The usage is Math.PI. I will no longer introduce
# one by one, please refer to the manual:
#
# Math Object
...
# rounding down to get a small integer; round() according to the fractional part
...
# want to keep two decimal places or otherwise, you should use Methods of Number
# object--toFixed(), toExponential() and toPrecision(), or use the following
# techniques:
#
# var n=1.23456
...
def round_it(n):
    left, right = (len(digits) for digits in str(n).split('.'))
    if left < right:
        return math.ceil(n)
    elif left > right:
        return math.floor(n)
